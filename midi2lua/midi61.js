document.getElementById("convertButton").addEventListener("click", async () => {
    ...
    outputText.value = "converting..."; // Clear the output text area
...
// Step 1: Collect Notes & Pedal Events Across All Tracks
midi.tracks.forEach(track => {
    ...
    if (document.getElementById("sustainPedalCheckBox").checked && track.controlChanges && track.controlChanges[64]) {
...
// Step 2: Combine, Sort, and Process Events
let allEvents = [...notesByTime, ...pedalEvents];
...
allEvents.forEach((event, index) => {
    let timeDelta = roundToThree((event.time - lastEndTime) * multBy);
    ...
    if (event.type === 'pedal') {
        ...
        const currentKey = noteMap[event.name] || event.name;
        let keypressDuration = 'x';
        let vel = roundToThree(event.velocity) || 0.5;

        if (lastVel !== vel && document.getElementById("velocityCheckbox").checked) {
            songscript += `adjustVelocity(${vel})\n`;
            lastVel = vel;
        }

        if (document.getElementById("shortNotesCheckbox").checked) {
            keypressDuration = roundToThree(event.duration * multBy);
        }
